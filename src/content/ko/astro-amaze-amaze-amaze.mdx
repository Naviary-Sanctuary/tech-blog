...
최근에 `프로젝트 헤일메리`를 아이맥스까지 가서 두 번이나 볼 정도로 아주 재미있게 봤다.

거기서 빛의 속도로 이동하는 **아스트로파지** 라는 미생물이 나오는데, '아스트로'파지 하니까 내 테크 블로그를 아스트로(Astro)로 만들었다는 사실과 내가 왜 이 기술을 골랐는지, 이것이 뭐가 좋은지를 안썼다는 사실을 깨달았다.

이런 좋은 기회는 못참지. 그래서 한번 써보려고 한다. 내가 왜 테크 블로그를 나누었고, 왜 테크 블로그의 동력원으로 아스트로를 사용했는지를.
